...
export function triggerMoltbotRestart() {
    ...
    if (process.platform !== "darwin") {
        ...
        if (process.platform === "win32") {
            const taskName = process.env.CLAWDBOT_WINDOWS_TASK_NAME ||
                resolveGatewayWindowsTaskName(process.env.CLAWDBOT_PROFILE);
            // Stop the task first
...
export function scheduleGatewaySigusr1Restart(opts) {
    ...
    // Windows does not support SIGUSR1; use event emitter only
    const isWindows = process.platform === "win32";
    const hasListener = process.listenerCount("SIGUSR1") > 0;
    setTimeout(() => {
        try {
            if (hasListener || isWindows) {
                // On Windows, always use emit since process.kill with SIGUSR1 is not supported
    ...
    return {
        ...
        delayMs,
        reason,
        mode: hasListener || isWindows ? "emit" : "signal",
    };
}
