...
|----------|--------|-------|
| macOS | ✅ Full Support | Recommended, includes menu bar app |
| Linux | ✅ Full Support | Uses systemd service |
| Windows (Native) | ✅ Full Support | Uses Windows Task Scheduler |
| Windows (WSL2) | ✅ Full Support | Recommended for advanced users |
...
Moltbot now fully supports **native Windows** (no WSL2 required).

### Prerequisites

1. Install **Node.js ≥22**: Download from [nodejs.org](https://nodejs.org/)
2. Install **pnpm**:
   ```powershell
   npm install -g pnpm
   ```
...
node moltbot.mjs config set gateway.auth.token "your-secure-token"
...
Then open the output URL in your browser (e.g., `http://127.0.0.1:18789/?token=your-token`)

### Configure AI Models

To use chat features, you need to configure AI model API keys:

#### Using Anthropic (Claude)

```powershell
node moltbot.mjs config set models.providers.anthropic.apiKey "your-api-key"
```

#### Using OpenAI

```powershell
node moltbot.mjs config set models.providers.openai.apiKey "your-api-key"
...
|---------|--------|-------|
| Gateway Service | ✅ | Uses Windows Task Scheduler |
...
| LINE | ✅ | Full support |
| iMessage | ❌ | macOS only |
| Browser Control | ✅ | Chrome/Edge/Brave |
| Control UI | ✅ | Full support |

### Windows Troubleshooting

#### 1. "bash not found" error during build

The project now uses cross-platform Node.js scripts. Make sure you're using the latest version.

#### 2. Gateway token error

You must set a token before starting:
```powershell
...
You need to configure AI model API keys (see "Configure AI Models" section above).
